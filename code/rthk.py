...
async def process_category(category, url):
    ...
    # feedImg = f"http://localhost:8080/?n=-1&ll&output=webp&url={urllib.parse.quote_plus('https://external-content.duckduckgo.com/ip3/' + urllib.parse.urlparse(categories_data[category]['url']).netloc + '.ico')}"
    feedImg = f"http://localhost:8080/?n=-1&output=webp&url={urllib.parse.quote_plus('https://favicone.com/' + urllib.parse.urlparse(categories_data[category]['url']).netloc)}"
    fg.logo(feedImg)
